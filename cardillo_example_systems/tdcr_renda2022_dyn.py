...
class Controller:
    def __init__(
        ...
        self._la_t = np.zeros(self.nla_tau, dtype=np.float64)
    ...
    def la_tau(self, t, q, u):
        ...
        M = self.rod.M(t, q).tocsr()[-6:-3, -6:-3]
        W_c = self.rod.W_c(t, q)
        la_c = self.rod.la_c(t, q, u)
        h = self.rod.h(t, q, u)[-6:-3]
        W_tau = self.W_tau(t, q).toarray(fix_size=True)[-6:-3]

        Wla_c = (W_c.tocsr(fix_size=True) @ la_c)[-6:-3]
        la_tau_comp = np.linalg.lstsq(W_tau, -(Wla_c + h * 0), rcond=1e-6)[0]
        la_tau_feedback = np.linalg.lstsq(
            W_tau,
            M @ ((r_OP_traj - r_OP) * self.Kp_r**2 + (-v_P) * 2 * self.Kp_r),
            rcond=1e-6,
        )[0]
    ...
    def assembler_callback(self):
        qDOF = self.qDOF = self.rod.qDOF
        uDOF = self.uDOF = self.rod.uDOF
