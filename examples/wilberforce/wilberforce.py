...
if __name__ == "__main__":
    ...
    elapsed_time = np.ones_like(t_eval) * elapsed_time

    from solve_dae.integrate._dae.radau import KAPPA
    np.savetxt(
        f"wilberforce_kappa_{KAPPA}.txt",
        np.vstack((t_eval, dt, elapsed_time)).T,
        delimiter=", ",
        header="t, dt, elapsed_time",
        comments="",
    )
    ...
    np.savetxt(
        f"wilberforce_solution.txt",
        np.vstack((t, r_OS[:, 2], np.rad2deg(angles[:, 0]))).T,
        delimiter=", ",
        header="t, z, alpha",
        comments="",
    )
