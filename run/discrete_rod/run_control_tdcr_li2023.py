...
def dydt(t, y):
    ...
    # control input
    n_tau = system_dyn.nla_tau
    r_OP = q[-7 - n_tau : -4 - n_tau]
    la_t_dot = dr_OP_dla_t_inv @ (r_des - r_OP) * Kp

    # update q_dot
    q_dot = system_dyn.q_dot(t, q, u)
    q_dot[-n_tau:] = la_t_dot

    # update u_dot
    h = system_dyn.h(t, q, u)
    W_c = rod_dyn.W_c(t, q[:-n_tau]).tocsr(fix_size=True)
    la_c = rod_dyn.la_c(t, q[:-n_tau], u)
    W_tau_coo = system_dyn.W_tau(t, q, format="Coo", coo=W_tau_coo)
    W_tau = W_tau_coo.tocsr(fix_size=True)
    la_tau = system_dyn.la_tau(t, q, u)
...
t, y =solve_ivp_sequence(dydt, y0, 0, t_sim, dt_sequence=1e-2, method="RK23", max_step=1e-3, step_callback=step_callback, ivp_callback=compute_dr_OP_dla_t)
...
sol_dyn = Solution(system_dyn, t, y[:, :nq])


###############
# visualization
###############
t, q = sol_dyn.t, sol_dyn.q
...
plotter.render_solution(sol_dyn, True)
